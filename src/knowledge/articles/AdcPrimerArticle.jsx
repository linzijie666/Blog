  ...
  return (
    <>
      ...
      <section id="architectures">
        ...
        <p><strong>FLASH 型</strong>：标准 n 位结构用 <FormulaText text="2^n-1" /> 个判决比较器并行比较，常见电阻串用 <FormulaText text="2^n" /> 个电阻产生门限，再由编码器一次输出。3bit FLASH 即 7 个比较器、8 个电阻；额外的溢出检测不能混入通用计数。</p>
        <ArticleFigure src="images/knowledge/analog-devices/adc-flash-architecture.webp" fullSrc="images/knowledge/analog-devices/adc-flash-architecture-hd.jpg" alt="3bit FLASH ADC 结构" caption="标准 FLASH：分压链 + 2^n−1 个判决比较器 + 编码器；3bit 对应 7 个比较器和常见的 8 段电阻串。" sourcePage="24" />
      ...
      <section id="resolution-lsb">
        ...
        <div className="formula-block"><figcaption>三个基本换算</figcaption><div className="formula"><FormulaText text="LSB=FSR/2^N，电压=数字量×LSB" /></div><p>12bit/3.3V：LSB=0.806mV，数字量 2048 → 1.65V，4095 → 3.299V（差 1LSB 到 Vref）。量程从 3.3V 改成 1.65V 会把理想 LSB 减半、提高量化分辨率，但总精度仍受偏移、增益、噪声、INL 和基准误差限制，不能保证“精度翻倍”。</p></div>
      ...
      <section id="sampling-interface">
        ...
        <p><strong>输入驱动</strong>不能只比一个“输入阻抗”数字。ADS1115 给出了随量程变化的共模/差分输入阻抗；STM32F103 这类 SAR ADC 则要按开关电容、外部源阻抗和采样时间分析动态建立。是否加缓冲取决于目标采样率、建立精度和信号源能力，缓冲器本身还要满足稳定性与输入输出范围。</p>
        ...
        <WorkedExample
          ...
          calculation={["严格按 ≤1mV/LSB，4.2V/4096≈1.025mV，因此 12bit 不能无条件满足，至少需要更高有效分辨率或缩小送入 ADC 的范围", "ADS1115 的 ±4.096V 档正满量程只有 4.096V；4.2V 大于 4.096V，必须先分压，且每个输入脚都要保持在 GND~VDD 的允许范围内", "例如把 4.2V 分压到 3.8V 以下，再按新的输入范围、125µV/LSB、分压比误差和 ENOB 做总误差预算"]}
          verification={["核对 ADS1115 绝对输入电压与 PGA 满量程是两个不同限制", "把分压电阻容差、输入偏置、噪声和参考/增益误差纳入总误差", "ADS1115 860SPS 对慢变电池足够；再比较内置 ADC+分压/缓冲方案的成本"]}
      ...
      <section id="interview">
        ...
        <div className="review-questions">
          ...
          <details><summary>四种 ADC 架构的优缺点和应用？</summary><p>SAR：结构简单、功耗低、通用采集常用；FLASH：最快、延迟最低，但标准 n 位需 2^n−1 个比较器，规模限制分辨率；流水线：速度与精度均衡、存在流水延迟；Σ-Δ：过采样+噪声整形+数字滤波，分辨率高但延迟较大。</p></details>

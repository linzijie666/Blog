    ...
    <>
      <section id="decoupling-layout">
        ...
        <div className="application-list">
          <article><h3>1. 单独分配，不能扎堆</h3><p>每个电源引脚至少分配一颗小容值去耦电容（10nF/100nF），不能把几颗电容摆在一起共用。STM32 这类芯片的每颗去耦电容都要尽可能靠近各自的电源引脚。</p></article>
      ...
      <section id="decoupling-radius">
        <h2>如何理解去耦半径</h2>
        <p>「小容值靠近引脚」的原理是<strong>去耦半径</strong>：电容能有效去耦的空间范围。电容存在等效串联电感 ESL，容值越大、封装越大，ESL 越大，自谐振频率越低。去耦半径约为<strong>电容自谐振频率对应波长的 1/40~1/50</strong>——频率越高波长越短，去耦半径越小，所以小容值高频电容必须贴着芯片放，远了来不及供电。</p>
        <div className="formula-block"><figcaption>去耦半径估算</figcaption><div className="formula"><FormulaText text="d_{decap} ≈ λ/40 ~ λ/50（λ 为自谐振频率对应波长）" /></div><p>大容值电容自谐振频率低、去耦半径大，可以放得远一些；小容值电容自谐振频率高、去耦半径小，必须紧贴引脚。</p></div>
        ...
        <WorkedExample
          ...
          calculation={["自谐振频率 f0 = 1/(2π√(LC)) ≈ 22MHz，对应波长约 13.6m（PCB 上按信号传播速度折算更短）", "去耦半径 = λ/40~λ/50，仅几十厘米量级，实际受走线电感限制更小", "引线每增加 1mm 约增加 1nH 电感，远放会明显抬高高频阻抗"]}
          verification={["示波器观察电源引脚纹波，电容位置由远改近后高频毛刺明显减小", "PDN 阻抗曲线在目标频段保持低阻"]}
          answer="去耦半径约为自谐振频率波长的 1/40~1/50，小容值高频电容的有效半径很小，必须就近贴着电源引脚布置；大容值电容半径大，可以放远些负责低频储能。"
        />
      </section>
      ...
      <section id="interview">
        ...
        <div className="review-questions">
          <details><summary>去耦电容布置有哪些原则？</summary><p>①每个电源引脚单独分配小容值电容，不能扎堆；②引线尽可能短（贴片优于插件，过孔方式影响寄生电感）；③多容值并联时小容值靠近引脚；④BGA 的去耦电容放引脚正下方背面。</p></details>
          <details><summary>什么是去耦半径？</summary><p>电容能有效去耦的空间范围，约为自谐振频率对应波长的 1/40~1/50。容值越大封装越大、ESL 越大、自谐振频率越低、去耦半径越大——所以小容值高频电容必须贴近芯片引脚，大容值电容可放远负责低频储能。</p></details>
